...
#define ROT_240_240_180                                                        \
  MADCTL::SCAN_ORDER | MADCTL::HORIZ_ORDER | MADCTL::COL_ORDER |               \
      MADCTL::ROW_ORDER
#define ROT_240_240_270                                                        \
  MADCTL::SWAP_XY | MADCTL::HORIZ_ORDER | MADCTL::ROW_ORDER
...
void ST7789::init(bool auto_init_sequence, bool round, uint32_t spi_baud) {
  ...
  // channel_config_set_dreq(&config, spi_get_index(spi) ? DREQ_SPI1_TX :
  // DREQ_SPI0_TX); dma_channel_configure(
  //   dma_channel, &config, &spi_get_hw(spi)->dr, frame_buffer, width * height,
  //   false);
}

spi_inst_t *ST7789::get_spi() const { return spi; }

uint ST7789::get_cs() const { return cs; }

uint ST7789::get_dc() const { return dc; }

uint ST7789::get_sck() const { return sck; }

uint ST7789::get_mosi() const { return mosi; }

uint ST7789::get_bl() const { return bl; }
...
void ST7789::update(bool dont_block) {
  command(reg::RAMWR, width * height * sizeof(uint16_t),
