...
namespace pimoroni {
PicoGraphics::PicoGraphics(uint16_t width, uint16_t height,
                           uint16_t *frame_buffer)
    : frame_buffer(frame_buffer), bounds(0, 0, width, height),
      clip(0, 0, width, height) {
  set_font(&font6);
};

void PicoGraphics::set_font(const Font *font) { this->font = font; }

void PicoGraphics::set_pen(uint8_t r, uint8_t g, uint8_t b) {
  pen = create_pen(r, g, b);
}

void PicoGraphics::set_pen(Pen p) { pen = p; }

void PicoGraphics::set_clip(const Rect &r) { clip = bounds.intersection(r); }

void PicoGraphics::remove_clip() { clip = bounds; }
...
void PicoGraphics::clear() { rectangle(clip); }
...
void PicoGraphics::pixel_span(const Point &p, int32_t l) {
  ...
  if (p.x + l < clip.x || p.x >= clip.x + clip.w || p.y < clip.y ||
      p.y >= clip.y + clip.h)
    return;
  ...
  if (clipped.x + l >= clip.x + clip.w) {
    l = clip.x + clip.w - clipped.x;
  }
...
void PicoGraphics::character(const char c, const Point &p, uint8_t scale) {
  ...
  Rect char_bounds(p.x, p.y, font->widths[char_index] * scale,
                   font->height * scale);

  if (!clip.intersects(char_bounds))
    return;
...
void PicoGraphics::triangle(Point p1, Point p2, Point p3) {
  Rect triangle_bounds(Point(std::min(p1.x, std::min(p2.x, p3.x)),
                             std::min(p1.y, std::min(p2.y, p3.y))),
                       Point(std::max(p1.x, std::max(p2.x, p3.x)),
...
void PicoGraphics::polygon(const std::vector<Point> &points) {
  static int32_t nodes[64]; // maximum allowed number of nodes per scanline for
                            // polygon rendering
  ...
  for (p.y = std::max(clip.y, miny); p.y <= std::min(clip.y + clip.h, maxy);
       p.y++) {
    uint8_t n = 0;
    for (uint16_t i = 0; i < points.size(); i++) {
      ...
      if ((sy < fy && ey >= fy) || (ey < fy && sy >= fy)) {
        ...
        int32_t px =
            int32_t(sx + float(fy - sy) / float(ey - sy) * float(ex - sx));

        nodes[n++] = px < clip.x
                         ? clip.x
                         : (px >= clip.x + clip.w
                                ? clip.x + clip.w - 1
                                : px); // clamp(int32_t(sx + float(fy - sy) /
                                       // float(ey - sy) * float(ex - sx)),
