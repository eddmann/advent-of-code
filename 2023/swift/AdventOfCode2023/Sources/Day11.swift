...
struct Day11: AdventDay {
  ...
  func part1() -> Any {
    ...
    return
      ...
      .map { (pair) -> Int in
        let totalExtraRows = extraRows.filter { y in
          (min(pair[0].y, pair[1].y)...max(pair[0].y, pair[1].y)).contains(y)
        }.count
        let totalExtraCols = extraCols.filter { x in
          (min(pair[0].x, pair[1].x)...max(pair[0].x, pair[1].x)).contains(x)
        }.count
  ...
  func part2() -> Any {
    ...
    return
      ...
      .map { (pair) -> Int in
        let totalExtraRows =
          extraRows.filter { y in
            (min(pair[0].y, pair[1].y)...max(pair[0].y, pair[1].y)).contains(y)
          }.count * 999_999
        let totalExtraCols =
          extraCols.filter { x in
            (min(pair[0].x, pair[1].x)...max(pair[0].x, pair[1].x)).contains(x)
          }.count * 999_999
        return manhattanDistance(pair[0], pair[1]) + totalExtraRows + totalExtraCols
      }
