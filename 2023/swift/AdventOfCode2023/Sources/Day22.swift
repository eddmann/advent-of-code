struct Day22: AdventDay {
  ...
  struct Brick {
    ...
    func isSupported(by brick: Brick) -> Bool {
      brick.isDirectlyBelow(brick: self)
        && (start.x...end.x).overlaps(brick.start.x...brick.end.x)
        && (start.y...end.y).overlaps(brick.start.y...brick.end.y)
    }
  ...
  func drop(bricks: [Brick]) -> [Brick] {
    ...
    for brick in bricks.sorted(by: { $0.bottom < $1.bottom }) {
      ...
      while true {
        ...
        if dropped.bottom > 0
          && settledSpace.intersection(dropped.volume).isEmpty
        {
          dropping = dropped
          continue
        }
      ...
      settledSpace = settledSpace.union(dropping.volume)
    }
  ...
  func canRemove(from bricks: [Brick], at index: Int) -> Bool {
    ...
    return
      ...
      .dropFirst(index + 1)
      .contains(where: {
        brick.isDirectlyBelow(brick: $0) && canDrop(from: volumeWithoutSubjectBrick, brick: $0)
      })
  }
  ...
  func part2() -> Any {
    ...
    return (0..<bricks.count)
      .sum { chainReaction(for: bricks, at: $0) }
  }
}
