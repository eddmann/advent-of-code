struct Day23: AdventDay {
  ...
  func compressToJunctions(
    within map: HikeMap, from start: HikePath, to end: HikePath,
    directionsForPath: (Character) -> [HikeDirection]
  ) -> JunctionsMap {
    let junctions = Set([start, end]).union(
      map.keys.filter { path in path.neighbours().filter { map.keys.contains($0) }.count > 2 })
  ...
  func part1() -> Any {
    ...
    let paths = compressToJunctions(
      within: map, from: start, to: end,
      directionsForPath: {
        switch $0 {
        case "^":
          return [.up]
