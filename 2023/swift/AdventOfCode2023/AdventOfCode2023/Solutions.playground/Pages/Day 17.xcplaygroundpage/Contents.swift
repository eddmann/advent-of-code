...
func simuluate(city: Dictionary<CruciblePoint, Int>, forwardsLimit: Int, rotateLimit: Int) -> Int {
    let goal = CruciblePoint(city.keys.map { $0.x }.max()!, city.keys.map { $0.y }.max()!)
    
    var queue = PriorityQueue<Crucible>()
    ...
    while let crucible = queue.dequeue() {
        ...
        if city[forwards] != nil && crucible.forwards < forwardsLimit {
            queue.add(Crucible(position: forwards, direction: crucible.direction, forwards: crucible.forwards + 1, heatLoss: crucible.heatLoss + city[forwards]!))
        }
        ...
        if city[left] != nil {
            queue.add(Crucible(position: left, direction: crucible.direction.rotateLeft(), forwards: 1, heatLoss: crucible.heatLoss + city[left]!))
        }
        
        let right = crucible.position.step(crucible.direction.rotateRight())
        if city[right] != nil {
            queue.add(Crucible(position: right, direction: crucible.direction.rotateRight(), forwards: 1, heatLoss: crucible.heatLoss + city[right]!))
        }
    }
