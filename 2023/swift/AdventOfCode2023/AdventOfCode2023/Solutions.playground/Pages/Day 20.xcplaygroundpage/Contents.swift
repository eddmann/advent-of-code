...
func partTwo() -> CustomStringConvertible {
    ...
    let connectionsToLastModule = modules.values.compactMap { $0.outgoing.contains(lastModuleId) ? $0.id : nil }
    var pressesToHighPulse = Dictionary(uniqueKeysWithValues: connectionsToLastModule.map { ($0, 0) })
